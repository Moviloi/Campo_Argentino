...
namespace CampoArgentino.Presentacion
{
    public partial class FormCliente : Form
    {
        ...
        public FormCliente()
        {
            ...
            this.ttMensaje.SetToolTip(this.txtCUIT, "Ingrese el CUIT del cliente");
            this.ttMensaje.SetToolTip(this.txtDireccion, "Ingrese la dirección del cliente");
            this.ttMensaje.SetToolTip(this.txtTelefono, "Ingrese el teléfono del cliente");
            this.ttMensaje.SetToolTip(this.txtEmail, "Ingrese el email del cliente");
        }
        ...
        private void OcultarColumnas()
        {
            ...
            if (this.dataListado.Columns.Contains("idcliente"))
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MensajeError("No hay clientes seleccionados para eliminar");
                    return;
                }

                DialogResult Opcion = MessageBox.Show(
                    $"¿Realmente desea eliminar los {cantidadSeleccionados} clientes seleccionados?\n\nEsta acción no se puede deshacer.",
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataListado.Rows)
                    {
                        if (!row.IsNewRow && row.Cells["Seleccionar"].Tag != null && (bool)row.Cells["Seleccionar"].Tag)
                        {
                            ...
                            string nombreCliente = row.Cells["Nombre"].Value?.ToString() ?? "Sin nombre";
                            ...
                            else
                            {
                                ...
                                {
                                    erroresDetallados.AppendLine($"{nombreCliente}: {Rpta}");
                                }
                            }
                        }
                    }
                    ...
                    if (errores == 0)
                    {
                        MensajeOk($"Se eliminaron correctamente {eliminadosExitosos} clientes");
                    }
                    else
                    {
        ...
        private void dataListado_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                this.txtClienteID.Text = Convert.ToString(this.dataListado.CurrentRow.Cells["idcliente"].Value);
